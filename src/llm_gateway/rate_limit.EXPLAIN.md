...
So you need a thing that answers one question, cheaply enough to ask before
every single call: *may I send this right now, and if not, how long until I may?*
...
goes to 0. All ten went out in a burst, which is the point of `C`.
...
bounds the average to `r`. The practical argument: it is what providers
