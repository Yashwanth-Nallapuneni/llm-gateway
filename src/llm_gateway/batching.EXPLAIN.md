# Batching — read this before `batching.py`
...
`max_wait_ms` near zero — a user is watching. An offline eval sweep over 50,000
...
**Should the batch timer start from the oldest or the newest request?** Oldest.
Newest lets a trickle reset the timer forever and starves the first arrival.

**What happens to a single request arriving into an empty queue?** Naively it
eats the full `max_wait_ms` for nothing. Fix it with adaptive dispatch — flush
when the queue is empty and nothing is in flight.

**What does increasing `max_wait_ms` buy, and what does it cost?** Buys fuller
batches: fewer round trips, better rate-limit utilisation, more throughput.
Costs every request that latency, straight onto p99.
