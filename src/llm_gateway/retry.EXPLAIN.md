# Retries and backoff — read this before `retry.py`
...
**Why is 400 not retryable but 429 is?** A `400` says the request is wrong —
identical bytes produce an identical failure forever. A `429` says the request
is fine but arrived too soon; the same bytes later succeed.

**Why does backoff need jitter?** Without it, clients that failed together retry
together, reproducing the overload. Jitter decorrelates the fleet.

**`Retry-After: 30`, your backoff says 2s — what do you wait?** Thirty. The
header is authoritative; `max()`, never `min()`.
